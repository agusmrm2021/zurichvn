const API_URL =
  "https://lemweccxac.execute-api.ap-southeast-1.amazonaws.com/uat/sendemail";
const TO_ADDRESS = "huy.do@merimen.com";
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (!/^\+?[\d\s\-().]{7,15}$/.test(phone)) {
      showAlert(
        "Please enter a valid contact number (7–15 digits, may include +, spaces, or dashes).",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const timer = setInterval(function () {
          countdown--;
          btn.textContent = "Submit (" + countdown + "s)";
          if (countdown <= 0) {
            clearInterval(timer);
            btn.disabled = false;
